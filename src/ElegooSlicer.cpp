...
int CLI::run(int argc, char **argv)
{
        ...
        for (const std::string& file : m_input_files) {
            ...
            try {
                ...
                if (is_bbl_3mf)
                {
                    ...
                    Semver cli_ver = *Semver::parse(ORCA_VERSION);
                    if (!allow_newer_file && ((cli_ver.maj() != file_version.maj()) || (cli_ver.min() < file_version.min()))){
                        BOOST_LOG_TRIVIAL(error) << boost::format("Version Check: File Version %1% not supported by current cli version %2%")%file_version.to_string() %ORCA_VERSION;
    ...
    for (auto const &opt_key : m_actions) {
        if (opt_key == "help") {
            ...
            m_print_config.save_to_json(m_config.opt_string(opt_key), std::string("project_settings"), std::string("project"), std::string(ORCA_VERSION));
