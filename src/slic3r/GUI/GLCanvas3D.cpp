    ...
    if (printer_technology == ptFFF && m_config->has("filament_colour") && (m_canvas_type != ECanvasType::CanvasAssembleView)) {
        ...
        if (wt && (timelapse_enabled || filaments_count > 1)) {
            for (int plate_id = 0; plate_id < n_plates; plate_id++) {
                ...
                // BBS: Use plate's actual extruder count instead of global filaments_count
                // to get correct wipe tower depth estimation
                std::vector<int> plate_extruders = ppl.get_plate(plate_id)->get_extruders(true);
                size_t plate_filaments_count = plate_extruders.size();
                // Fallback to global count if plate has no extruders yet
                if (plate_filaments_count == 0) {
                    plate_filaments_count = filaments_count;
                }
                const auto& wipe_tower_data = print->wipe_tower_data(plate_filaments_count);
...
void GLCanvas3D::_render_imgui_select_plate_toolbar()
{
    ...
    if(m_sel_plate_toolbar.show_stats_item)
    {
        ...
        if (all_plates_stats_item->slice_state == IMToolbarItem::SliceState::UNSLICED || all_plates_stats_item->slice_state == IMToolbarItem::SliceState::SLICING) {
            ...
            ImGui::GetWindowDrawList()->AddRectFilled(bar_bg_bgn, bar_fg_end, IM_COL32(22, 119, 255, 255),   bar_size.y); // BAR PROGRESS
        }
        else if (all_plates_stats_item->slice_state == IMToolbarItem::SliceState::SLICE_FAILED) {
