...
- Code and documentation all in English, variables/parameters/return values explicitly typed
- Write clear comments for classes, methods, and critical logic
- English comments explain "why" not "what", avoid restating code
- Comments must describe objective facts such as **intent, constraints, ownership, and threading rules**. Do not include collaboration/process background (e.g., "per AI discussion", "per chat history", "as requested in conversation").
- If a fix needs a reason in a comment, use traceable context (e.g., `// Fix: <brief reason> (ISSUE-ID)`), avoid conversational/explanatory narrative.
